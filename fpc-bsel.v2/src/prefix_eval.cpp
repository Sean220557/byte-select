...
static int evaluate_raw_stream(const char* model_path,const char* input_path,int entries,std::uint64_t progress){
  ...
  Cache cache(entries);std::vector<fpc_bsel::TierReorderRegion> tier_regions; tier_regions.reserve(regions);std::vector<std::uint16_t> mcc_sizes,mcc_sizes_before;mcc_sizes.reserve(count);mcc_sizes_before.reserve(count);std::uint64_t hits=0,lookups=0,selected=0,crossed=0,pair_physical_before=0,pair_physical_after=0;std::array<std::uint64_t,5> trans{},tier_algorithm_saved{},tier_quantized_saved{};std::array<std::uint64_t,4> pair_regions{},pair_bad_before{},pair_bad_after{},pair_swaps{};std::uint64_t ab=0,aa=0,qb=0,qa=0;auto started=std::chrono::steady_clock::now();std::array<std::uint8_t,4096> raw{};
  ...
  std::uint64_t metadata_bytes=regions*34ULL;for(const auto n:pair_swaps)metadata_bytes+=n;for(auto& n:mcc_sizes){const auto room=std::uint64_t(256U-n);const auto add=std::min(room,metadata_bytes);n=static_cast<std::uint16_t>(n+add);metadata_bytes-=add;if(!metadata_bytes)break;}if(metadata_bytes)throw std::runtime_error("MCC metadata does not fit in region records");
  if(const char* path=std::getenv("PREFIX_MCC_SIZES")){std::ofstream sizes(path);if(!sizes)throw std::runtime_error("cannot open MCC size output");for(const auto n:mcc_sizes)sizes<<n<<'\n';}
  if(const char* path=std::getenv("PREFIX_MCC_SIZES_BEFORE")){std::ofstream sizes(path);if(!sizes)throw std::runtime_error("cannot open pre-swap MCC size output");for(const auto n:mcc_sizes_before)sizes<<n<<'\n';}
