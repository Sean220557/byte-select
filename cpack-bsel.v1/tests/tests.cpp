...
int main() try {
    ...
    check(fpc_bsel::cpack_decode(fpc_bsel::cpack_encode(zeros)) == zeros,
          "zero C-Pack round trip");
    check(fpc_bsel::cpack_decode(fpc_bsel::cpack_encode(patterns)) == patterns,
          "all-pattern C-Pack round trip");
    check(fpc_bsel::pack_cpack_tags(fpc_bsel::split_cpack(patterns).tags).size() == 6,
          "packed prefix size");
    ...
    check(fpc_bsel::decode_block(zero_encoded.bytes, model) == zeros, "zero codec round trip");
    ...
    std::cout << "all cpack-bsel tests passed\n";
    return 0;
