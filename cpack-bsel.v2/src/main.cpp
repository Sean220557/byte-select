...
int main(int argc, char** argv) try {
    ...
    if (command == "evaluate" || command == "roundtrip") {
        ...
        std::uint64_t raw = 0, fpc = 0, combined = 0, prefix = 0, residual = 0, inline_prefix = 0, raw_bsel = 0;
        for (const auto& block : blocks) {
            const auto encoded = fpc_bsel::encode_block(block, model);
            ...
            else if (encoded.mode == fpc_bsel::BlockMode::RawBsel) { ++raw_bsel; ++residual; }
