param(
    [string]$Root = "..",
    [string]$OutputDir = "../results/spark-flink/fpc-bsel-all"
)
...
$baselines = @("fpc", "bdi", "hybrid", "cpack", "bpc", "zstd", "lz4", "lz77-lite", "huffman")
...
foreach ($workload in $workloads) {
    ...
    $fpcModel = Join-Path $outputPath "$($workload.Name)-fpc-bsel.model"
    & $fpcBsel train $train $fpcModel --max-patterns 256 |
        Set-Content (Join-Path $outputPath "$($workload.Name)-fpc-bsel-train.log")
    if ($LASTEXITCODE -ne 0) { throw "FPC-BSEL training failed: $($workload.Name)" }
    $fpcOutput = & $fpcBsel evaluate $fpcModel $test
    if ($LASTEXITCODE -ne 0) { throw "FPC-BSEL evaluation failed: $($workload.Name)" }
    $fpcOutput | Set-Content (Join-Path $outputPath "$($workload.Name)-fpc-bsel-evaluate.log")
    $fpcValues = Parse-Line $fpcOutput
    $fpcValues.compressed_fraction = 1 - ([double]$fpcValues.raw_blocks / [double]$fpcValues.blocks)
    ...
    foreach ($preset in $presets) {
        ...
        Add-Row $workload.Name $preset "fpc-bsel" $fpcValues ([uint64]$fpcValues.physical_encoded_bytes)
    }
}
