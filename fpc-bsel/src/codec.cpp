...
std::size_t bsel_stream_size(const Bytes& data, std::size_t offset, const BselModel& model) {
    if (offset + 2 > data.size()) throw std::runtime_error("truncated BSEL segment");
    const auto id = data[offset] | (static_cast<std::size_t>(data[offset + 1]) << 8U);
    if (id >= model.patterns.size()) throw std::runtime_error("unknown BSEL pattern id");
    return 2 + model.patterns[id].rank();
}
...
EncodedBlock encode_block(const Bytes& block, const Model& model) {
    ...
    if (const auto encoded = bsel_encode(expanded_tags(parts.tags), model.prefix);
    ...
    if (const auto encoded = bsel_encode(parts.residual_block, model.residual);
