...
codebook to 192 patterns. Each pattern is 16B, so the hardware pattern SRAM
is 3072B. The serialized model adds a 28B file header.

Training uses the 16.77MB Fashion training sample. Independent evaluation uses
the 31.36MB Fashion test set. The measured lossless encoded ratio is 3.5964.
